...
async function pickData(url:URL,env:Env,user:User){const poolId=url.searchParams.get("poolId")||"",week=clampWeek(url.searchParams.get("week"));if(!await isMember(env,poolId,user.id))return json({error:"Not a member."},403);const mine=(await env.DB.prepare("SELECT game_id,team_id,admin_adjusted FROM picks WHERE pool_id=? AND user_id=?").bind(poolId,user.id).all<any>()).results,people=(await env.DB.prepare("SELECT u.id user_id,COALESCE(u.display_name,u.username) display_name FROM memberships m JOIN users u ON u.id=m.user_id WHERE m.pool_id=?").bind(poolId).all<any>()).results,all=(await env.DB.prepare("SELECT p.user_id,p.team_id,p.game_id,g.week,g.completed,g.winner_id FROM picks p JOIN games g ON g.id=p.game_id WHERE p.pool_id=?").bind(poolId).all<any>()).results,ties=(await env.DB.prepare("SELECT user_id,week,guess FROM tiebreakers WHERE pool_id=?").bind(poolId).all<any>()).results;
 ...
 return json({picks:Object.fromEntries(mine.map(p=>[p.game_id,p.team_id])),adjusted:Object.fromEntries(mine.map(p=>[p.game_id,!!p.admin_adjusted])),tiebreaker:ties.find(t=>t.user_id===user.id&&t.week===week)?.guess??null,tiebreakerActual:actual,standings,history});
}
