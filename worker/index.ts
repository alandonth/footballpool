...
async function api(req:Request,env:Env,ctx:ExecutionContext,url:URL){
 ...
 if(url.pathname==="/api/settings"&&req.method==="GET")return appSettings(env);
 if(url.pathname==="/api/settings"&&req.method==="PATCH")return updateAppSettings(req,env,user);
 ...
 if(url.pathname==="/api/current-week"&&req.method==="GET")return currentWeek(env);
 ...
 if(url.pathname==="/api/tiebreaker"&&req.method==="POST")return saveTiebreaker(req,env,user);
...
async function updateMe(req:Request,env:Env,user:User){const b=await body(req),theme=["stadium","aquatic","blizzard","liberty","flight"].includes(String(b.theme))?String(b.theme):user.theme,display=cleanDisplay(b.displayName);await env.DB.prepare("UPDATE users SET display_name=?,theme=? WHERE id=?").bind(display,theme,user.id).run();return json({ok:true})}
async function appSettings(env:Env){const row=await env.DB.prepare("SELECT value FROM app_settings WHERE key='logo_mode'").first<any>();return json({logoMode:row?.value==="badge"?"badge":"espn"})}
async function updateAppSettings(req:Request,env:Env,user:User){if(user.role!=="admin")return json({error:"Admin required."},403);const b=await body(req),logoMode=b.logoMode==="badge"?"badge":"espn";await env.DB.prepare("INSERT INTO app_settings(key,value) VALUES('logo_mode',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value").bind(logoMode).run();return json({logoMode})}
...
async function changePool(req:Request,env:Env,user:User){const b=await body(req);if(b.action==="create"){const name=String(b.name||"").trim().slice(0,50);if(!name)return json({error:"Pool name required."},400);const pool={id:crypto.randomUUID(),name,invite_code:inviteCode(),owner_id:user.id,season:SEASON,created_at:Date.now()};await env.DB.batch([env.DB.prepare("INSERT INTO pools(id,name,invite_code,owner_id,season,created_at) VALUES(?,?,?,?,?,?)").bind(...Object.values(pool)),env.DB.prepare("INSERT INTO memberships(id,pool_id,user_id,joined_at) VALUES(?,?,?,?)").bind(crypto.randomUUID(),pool.id,user.id,Date.now())]);return json({pool})}if(b.action==="join"){const pool=await env.DB.prepare("SELECT * FROM pools WHERE invite_code=?").bind(String(b.inviteCode||"").toUpperCase()).first<any>();if(!pool)return json({error:"Invite code not found."},404);await env.DB.prepare("INSERT OR IGNORE INTO memberships(id,pool_id,user_id,joined_at) VALUES(?,?,?,?)").bind(crypto.randomUUID(),pool.id,user.id,Date.now()).run();return json({pool})}if(b.action==="leave"){const pool=await env.DB.prepare("SELECT * FROM pools WHERE id=?").bind(String(b.poolId||"")).first<any>();if(!pool||!await isMember(env,pool.id,user.id))return json({error:"Pool not found."},404);const next=pool.owner_id===user.id?await env.DB.prepare("SELECT user_id FROM memberships WHERE pool_id=? AND user_id<>? ORDER BY joined_at LIMIT 1").bind(pool.id,user.id).first<any>():null;if(pool.owner_id===user.id&&!next){await env.DB.prepare("DELETE FROM pools WHERE id=?").bind(pool.id).run()}else{const stmts=[env.DB.prepare("DELETE FROM picks WHERE pool_id=? AND user_id=?").bind(pool.id,user.id),env.DB.prepare("DELETE FROM tiebreakers WHERE pool_id=? AND user_id=?").bind(pool.id,user.id),env.DB.prepare("DELETE FROM memberships WHERE pool_id=? AND user_id=?").bind(pool.id,user.id)];if(next)stmts.unshift(env.DB.prepare("UPDATE pools SET owner_id=? WHERE id=?").bind(next.user_id,pool.id));await env.DB.batch(stmts)}return json({left:true})}return json({error:"Invalid action."},400)}

async function currentWeek(env:Env){try{const response=await fetch(`${ESPN}?xhr=1&dates=${SEASON}`,{headers:{accept:"application/json","user-agent":"FamilyPickem/1.0"}});if(response.ok){const payload:any=await response.json(),data=payload.content?.sbData||payload,week=Number(data.week?.number);if(week>=1&&week<=18)return json({week})}}catch(error){console.error("Current week lookup failed",error)}const row=await env.DB.prepare("SELECT week FROM games WHERE season=? AND start_time<=? ORDER BY start_time DESC LIMIT 1").bind(SEASON,new Date(Date.now()+86400000).toISOString()).first<any>();return json({week:clampWeek(row?.week||1)})}
...
async function schedule(url:URL,env:Env,ctx:ExecutionContext){
 ...
 const records=await teamRecords(env);
 return json({
  week,
  games:games.map(g=>gameDto(g,records)),
...
async function teamRecords(env:Env){const games=(await env.DB.prepare("SELECT home_id,away_id,winner_id FROM games WHERE season=? AND completed=1").bind(SEASON).all<any>()).results,records:Record<string,{wins:number;losses:number;ties:number}>={};const get=(id:string)=>records[id]??(records[id]={wins:0,losses:0,ties:0});for(const g of games){const home=get(g.home_id),away=get(g.away_id);if(!g.winner_id){home.ties++;away.ties++}else if(g.winner_id===g.home_id){home.wins++;away.losses++}else{away.wins++;home.losses++}}return records}
...
async function pickData(url:URL,env:Env,user:User){const poolId=url.searchParams.get("poolId")||"",week=clampWeek(url.searchParams.get("week"));if(!await isMember(env,poolId,user.id))return json({error:"Not a member."},403);const mine=(await env.DB.prepare("SELECT game_id,team_id,admin_adjusted FROM picks WHERE pool_id=? AND user_id=?").bind(poolId,user.id).all<any>()).results,people=(await env.DB.prepare("SELECT u.id user_id,COALESCE(u.display_name,u.username) display_name FROM memberships m JOIN users u ON u.id=m.user_id WHERE m.pool_id=?").bind(poolId).all<any>()).results,all=(await env.DB.prepare("SELECT p.user_id,p.team_id,p.game_id,g.week,g.completed,g.winner_id,g.start_time,g.lock_override FROM picks p JOIN games g ON g.id=p.game_id WHERE p.pool_id=?").bind(poolId).all<any>()).results,ties=(await env.DB.prepare("SELECT user_id,week,guess FROM tiebreakers WHERE pool_id=?").bind(poolId).all<any>()).results;
 const lastGame=await env.DB.prepare("SELECT * FROM games WHERE season=? AND week=? ORDER BY start_time DESC LIMIT 1").bind(SEASON,week).first<any>(),actual=lastGame?.completed?Number(lastGame.home_score)+Number(lastGame.away_score):null;
 const standings=people.map(p=>{const pp=all.filter(x=>x.user_id===p.user_id&&x.completed),wp=pp.filter(x=>x.week===week),guess=ties.find(t=>t.user_id===p.user_id&&t.week===week)?.guess??null;return{...p,correct:pp.filter(x=>x.team_id===x.winner_id).length,total:pp.length,weekCorrect:wp.filter(x=>x.team_id===x.winner_id).length,weekTotal:wp.length,tiebreakerGuess:p.user_id===user.id||actual!=null?guess:null,tiebreakerDifference:actual==null||guess==null?null:Math.abs(guess-actual)}}).sort((a,b)=>b.correct-a.correct||a.display_name.localeCompare(b.display_name));
 const history=[];for(let w=1;w<=18;w++){const complete=await env.DB.prepare("SELECT COUNT(*) c,SUM(completed) done FROM games WHERE season=? AND week=?").bind(SEASON,w).first<any>();if(!complete?.c||Number(complete.done)!==Number(complete.c))continue;const games=all.filter(x=>x.week===w&&x.completed),final=await env.DB.prepare("SELECT home_score,away_score FROM games WHERE season=? AND week=? ORDER BY start_time DESC LIMIT 1").bind(SEASON,w).first<any>(),totalPoints=Number(final?.home_score)+Number(final?.away_score);const scores=people.map(p=>{const guess=ties.find(t=>t.user_id===p.user_id&&t.week===w)?.guess??null;return{displayName:p.display_name,score:games.filter(x=>x.user_id===p.user_id&&x.team_id===x.winner_id).length,total:games.filter(x=>x.user_id===p.user_id).length,tiebreakerGuess:guess,difference:guess==null?null:Math.abs(guess-totalPoints)}}),max=Math.max(...scores.map(s=>s.score)),top=scores.filter(s=>s.score===max&&s.total>0),withGuesses=top.filter(s=>s.difference!=null),best=withGuesses.length?Math.min(...withGuesses.map(s=>Number(s.difference))):null;history.push({week:w,winners:best==null?top:top.filter(s=>s.difference===best),totalPoints})}
 const revealedPicks=all.filter(p=>p.week===week&&Date.now()>=new Date(p.lock_override||p.start_time).getTime()).map(p=>({userId:p.user_id,gameId:p.game_id,teamId:p.team_id}));
 return json({picks:Object.fromEntries(mine.map(p=>[p.game_id,p.team_id])),adjusted:Object.fromEntries(mine.map(p=>[p.game_id,!!p.admin_adjusted])),revealedPicks,tiebreaker:ties.find(t=>t.user_id===user.id&&t.week===week)?.guess??null,tiebreakerActual:actual,standings,history});
}
...
async function saveTiebreaker(req:Request,env:Env,user:User){const b=await body(req),week=clampWeek(b.week),guess=Number(b.guess);if(!await isMember(env,String(b.poolId||""),user.id))return json({error:"Not a member."},403);if(!Number.isInteger(guess)||guess<0||guess>200)return json({error:"Enter a total from 0 to 200."},400);const last=await env.DB.prepare("SELECT start_time,lock_override FROM games WHERE season=? AND week=? ORDER BY start_time DESC LIMIT 1").bind(SEASON,week).first<any>();if(!last)return json({error:"Schedule not available."},409);if(Date.now()>=new Date(last.lock_override||last.start_time).getTime())return json({error:"The tiebreaker is locked."},409);await env.DB.prepare("INSERT INTO tiebreakers(pool_id,user_id,week,guess,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(pool_id,user_id,week) DO UPDATE SET guess=excluded.guess,updated_at=excluded.updated_at").bind(b.poolId,user.id,week,guess,Date.now()).run();return json({saved:true})}
...
function gameDto(g:any,records:Record<string,{wins:number;losses:number;ties:number}>={}){const record=(id:string)=>{const r=records[id]||{wins:0,losses:0,ties:0};return r.ties?`${r.wins}-${r.losses}-${r.ties}`:`${r.wins}-${r.losses}`};return{id:g.id,startTime:g.start_time,lockTime:g.lock_override||g.start_time,status:g.status,completed:!!g.completed,winnerId:g.winner_id,home:{id:g.home_id,abbreviation:g.home_abbr,name:g.home_name,logo:g.home_logo,score:g.home_score,record:record(g.home_id)},away:{id:g.away_id,abbreviation:g.away_abbr,name:g.away_name,logo:g.away_logo,score:g.away_score,record:record(g.away_id)}}}
